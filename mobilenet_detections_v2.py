...
def build_cmd(tool):
    ...
    if tool == "raspivid":
        return [tool, "-o", "-", "-t", str(DURATION_MS), "-w", str(WIDTH), "-h", str(HEIGHT), "-fps", str(FPS)]
    # fallback: rpicam-vid (no size args)
    return [tool, "-o", "-", "-t", str(DURATION_MS)]
    
def stream_and_decode():
    tool = find_tool()
    if not tool:
        print("No capture tool found (libcamera-vid/raspivid/rpicam-vid)."); return
    vid_cmd = build_cmd(tool)
    ff_cmd = ["ffmpeg","-hide_banner","-loglevel","error","-f","h264","-i","pipe:0",
    ...
    with subprocess.Popen(vid_cmd, stdout=subprocess.PIPE) as vid_proc, \
        ...
        try:
            ...
            ff_proc.terminate(); vid_proc.terminate()
            ff_proc.wait(timeout=1); vid_proc.wait(timeout=1)
